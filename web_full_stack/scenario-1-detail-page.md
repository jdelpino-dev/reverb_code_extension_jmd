...
> **Acknowledging the contradiction:** The API report notes that Reverb enforces link-driven design — "clients must not construct URLs." This is a *deliberate, justified deviation*. HATEOAS link-following assumes first-party clients that always begin navigation from a root resource and traverse inward. As a third-party consumer with independent entry points (bookmarks, shared links, direct URLs), we cannot guarantee a prior API response exists to extract links from. The deviation is intentional, not an oversight.
...
2. **Security (SSRF risk)** — blindly following URLs from an external API response means your server will request whatever URL that response contains. An attacker who compromises the upstream API (or injects into the response) could redirect your server to internal services. Constructing from a known base URL + ID eliminates this. (Note: this is defense-in-depth. If the upstream API is compromised enough to inject malicious `_links`, you likely have bigger problems — but eliminating the vector is still worthwhile.)
...
4. **Stable API** — Reverb's URL structure is versioned. It won't change without a major version bump. The HATEOAS benefit of "the server can change URLs and clients adapt" doesn't apply to a third-party API you don't control the versioning of. (Counter-argument: if they *did* change URLs, our constructed URLs would break while link-following would still work. We accept that coupling — we'd update the client when we bump the API version anyway.)

**Middle ground — validated link-following:** You *could* follow `_links.self.href` with SSRF protection by allowlisting scheme (`https`) + hostname (`api.reverb.com`). This preserves HATEOAS benefits while eliminating the attack vector. Worth mentioning as the "if I controlled the architecture" option, but still requires a fallback construction path for direct-access routes — so you end up with two code paths regardless.
...
**Interview framing:** "My API report notes that Reverb's design philosophy is link-driven — 'clients must not construct URLs.' I'm deviating deliberately because we're a third-party consumer, not a first-party navigation client. We have entry points (bookmarks, direct URLs) that don't originate from an API response, so link-following can't be the only path. I *could* validate-and-follow links (allowlist `https://api.reverb.com`) as a middle ground, but the additional complexity isn't justified when URL construction works and the API is versioned. One code path, no SSRF surface, no conditional logic."
