...
This document describes the **original upstream codebase** as it will appear
when the interviewer presents it. Use it to orient quickly and identify issues
to discuss. Improvements you would make are noted inline as observations.

---

## Architecture Overview

This app uses a **layered architecture** (partially implemented):
...
| Service | `app.py` (`_load_categories`, `_search_categories`) | App logic: filtering, coordination |
...
This is the main architectural issue to notice and discuss. The
`_load_categories()` helper is a thin wrapper now, but it represents the service
boundary where pagination, caching, error handling, and normalization belong.
`listings` skips this layer entirely.

**What to say:** "I'd add `_load_listings()` for consistency -- it establishes
the service boundary for both routes. Then pagination, caching, and error
handling all have a clear home."
...
- `listings()`: Controller/route. **Calls client directly** (inconsistent -- no service helper).
...
class ReverbClient:
    ...
    def _get(self, path, params={}):  # Shared HTTP logic -- NOTE THE BUG
```

**Things to notice and articulate:**

- **Mutable default argument** -- `params={}` is a classic Python footgun.
  Default mutable objects are evaluated once at function definition time. If
  anyone mutates `params` inside `_get`, all subsequent calls see the mutation.
  The safe pattern is `params=None` then `if params is None: params = {}`.
- **No error handling** -- no try/except, no status code checks, no
  `raise_for_status()`. If the API returns a 500, `.json()` will try to parse
  HTML and raise a misleading `JSONDecodeError`. Happy path only.
- **No timeout** -- `requests.get` will hang indefinitely on a stalled
  connection. A user-facing page cannot wait forever.
- `base_uri` is injectable via constructor -- enables testing with a different
  host if needed (though tests mock at `requests.get` level instead).
- No auth needed -- only `Accept`, `Accept-Version`, `Content-Type` headers.
...
- `categories.html`: Renders filtered category list as `<ul class="list-group">`.
  Has a search form (GET) and a conditional "no results" message.
...
This deeply nested access has **no safety** -- a listing with no photos will
crash the template with a `KeyError`/`IndexError`.
...
- Error cases (API down, 404, timeout, malformed JSON)
- Empty states (no listings returned, no photos)
- Pagination or `per_page` behavior
- The `_search_categories` filter logic in isolation (only tested through route)
- The "no results" message rendering
...
| --- | --- | --- |
| Error handling | Service layer (try/except) + flash messages in routes | High -- fix first |
| Timeout | Client (`_get` method): `timeout=(3, 5)` | High -- one line |
| `raise_for_status()` | Client (`_get` method): after `requests.get()` | High -- one line |
| `params=None` fix | Client (`_get` method): safe default | High -- one line |
| `_load_listings()` | Service layer in `app.py` | High -- consistency |
| Pagination | Service layer (`_load_*` helpers) + query params | Medium |
| Caching | Service layer (e.g., `@lru_cache` on `_load_categories`) | Medium |
| Listing detail page | New route + client method + template | Medium |
| Input validation | Routes/controller (validate query params) | Low |
| Auth | Not needed -- Reverb public API is read-only | N/A |
...
- **No service helper strictly needed** -- fetching a single listing by ID is a
  straight client-to-route pass-through (like current listings). No
  filtering/coordination logic.
- **But** if you skip adding `_load_listing(id)`, you perpetuate the
  inconsistency. Say: "I'll add it for consistency," then keep it thin.
...
- **This is where you fix the inconsistency.** The main architectural issue is
  "listings skips the service layer." Adding `_search_listings(query)` or a
  `query` param to a new `_load_listings()` mirrors `_search_categories(query)`
  -- the architecture tells you exactly where it goes.
- **Decision point:** client-side filter (like categories -- fetch ALL, then
...
- **Implication:** If you choose API-side filtering, the service helper passes
  the query through to the client. If client-side, it becomes a
  `_load_listings()` + filter pattern (matching categories).

### Scenario 3: Pagination

- **Explicitly belongs in the service layer** -- this is the scenario that
  justifies `_load_listings()` existing as a service boundary.
...
- **The mutable default arg becomes real:** `_get(self, path, params={})` -- if
  you add `params['page'] = page` inside a caller that passes the same dict, the
  default persists across calls. This is where the gotcha stops being theoretical.
...
- **No input validation** (noted as missing). Category slugs from the API are
  safe, but if you accept user-typed slugs in the URL, you're passing untrusted
  input directly to the external API.
...
- **The codebase is most explicit about this gap:** "No error handling: no
  ...
  routes."
- **`_get` has no status check** -- `response.json()` will raise
  `JSONDecodeError` on non-JSON error responses (e.g., HTML error pages from a
  proxy).
...
- **Raw dicts, no Model** -- `listing['price']['amount']` requires nested keys to
  ...
  crash `float()`. No data class validates the shape.
- Sort by `price.amount_cents` (integer), not `price.amount` (string) -- avoids
  lexicographic ordering and float precision issues.
...
Scenario 6: Sort + pagination + error handling = 20 lines in one route function.
```
